...
import { NAMO_CONTENT_KINDS, NamoContent, NamoContentInput, NamoContentKind, NamoContentStatus, namoContentApi } from "@/lib/namoContentApi";
import { useAppSelector } from "@/store/hooks";

const EMPTY: NamoContentInput = { kind: "BLOG", slug: "", title: "", payload: {}, status: "ACTIVE", order: 0 };
const label = (kind: string) => kind.toLowerCase().replaceAll("_", " ").replace(/\b\w/g, (character) => character.toUpperCase());

export default function NamoCmsPage() {
  const organisationCode = useAppSelector((state) => state.scope.selectedOrganisationCode);
  const [kind, setKind] = useState<NamoContentKind | "">("");
  const [status, setStatus] = useState<NamoContentStatus | "">("");
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<NamoContentInput>(EMPTY);
  const [payloadText, setPayloadText] = useState("{}");
  const [jsonError, setJsonError] = useState("");
  const [uploading, setUploading] = useState(false);
  const filteredApi = useMemo(() => ({ ...namoContentApi, list: () => namoContentApi.list(kind || undefined, status || undefined) }), [kind, status]);
  ...
  const openCreate = () => { setEditingId(null); setForm({ ...EMPTY, kind: kind || "BLOG" }); setPayloadText("{}"); setJsonError(""); setError(""); setModalOpen(true); };
  const openEdit = (entry: NamoContent) => { setEditingId(entry._id); setForm({ kind: entry.kind, slug: entry.slug ?? "", title: entry.title ?? "", payload: entry.payload, status: entry.status, order: entry.order }); setPayloadText(JSON.stringify(entry.payload, null, 2)); setJsonError(""); setError(""); setModalOpen(true); };
  ...
  const handleUpload = async (file?: File) => {
    ...
    try {
      const uploaded = await namoContentApi.upload(file);
  ...
  const columns: Column<NamoContent>[] = [
  ...
  if (organisationCode !== "NAMOGANGE") return <div className="rounded-lg border border-surface-border bg-surface-card p-8 text-center"><FileJson2 className="mx-auto h-8 w-8 text-text-muted" /><h1 className="mt-3 font-semibold">Namo Gange CMS</h1><p className="mt-1 text-sm text-text-muted">Select Namo Gange to manage its website content.</p></div>;
  return <div className="space-y-4">
    <div className="flex items-center justify-between"><div><h1 className="text-lg font-semibold">Namo Gange CMS</h1><p className="text-xs text-text-muted">Manage all legacy Namo website content types in one scoped workspace.</p></div><Button size="sm" onClick={openCreate}><Plus className="h-4 w-4" /> New Content</Button></div>
    <div className="grid max-w-2xl gap-3 sm:grid-cols-2"><Select label="Content type" value={kind} onChange={(e) => setKind(e.target.value as NamoContentKind | "")}><option value="">All types</option>{NAMO_CONTENT_KINDS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</Select><Select label="Status" value={status} onChange={(e) => setStatus(e.target.value as NamoContentStatus | "")}><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></Select></div>
    <Table columns={columns} rows={rows} rowKey={(entry) => entry._id} loading={loading} emptyMessage="No Namo content found." onRowClick={openEdit} />
    <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? "Edit Content" : "New Content"} size="lg" footer={<><Button variant="secondary" size="sm" onClick={() => setModalOpen(false)}>Cancel</Button>{editingId && <Button variant="danger" size="sm" onClick={handleDelete}><Trash2 className="h-3.5 w-3.5" /> Delete</Button>}<Button size="sm" loading={saving} onClick={handleSave}>Save</Button></>}>
      <div className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><Select label="Content type" value={form.kind} onChange={(e) => setForm({ ...form, kind: e.target.value as NamoContentKind })}>{NAMO_CONTENT_KINDS.map((item) => <option key={item} value={item}>{label(item)}</option>)}</Select><Select label="Status" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as NamoContentStatus })}><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></Select><Input label="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /><Input label="Slug or page path" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value.toLowerCase().replace(/^\/+/, "").replace(/\s+/g, "-") })} /><Input label="Display order" type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} /></div>
