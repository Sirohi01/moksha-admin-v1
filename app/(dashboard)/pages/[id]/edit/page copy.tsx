...
import { PUBLIC_SITE_URL, cmsPages, cmsPagesFromSettings } from "@/lib/cmsPages";
...
export default function EditPage() {
  ...
  useEffect(() => {
    settingsApi.get().then((settings) => {
      const allPages = cmsPagesFromSettings(settings as any);
      const page = allPages.find(p => p.id === Number(params.id)) || cmsPages.find(p => p.id === Number(params.id));
      if (page) {
        setForm(prev => ({
          ...prev,
          ...
          content: "Loading content editor..."
        }));
      }
    }).catch(() => undefined);
  }, [params.id]);
