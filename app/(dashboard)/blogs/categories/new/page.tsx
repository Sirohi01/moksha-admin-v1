...
function FieldLabel({ children, required, optional }: { children: React.ReactNode; required?: boolean; optional?: boolean }) {
  return (
    <label className="mb-[6px] block text-[10px] font-extrabold text-[#24345e]">
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button type="button" onClick={onChange} className={`relative h-[22px] w-[38px] rounded-full ${checked ? "bg-[#087342]" : "bg-[#d7dde6]"}`}>
      <span className={`absolute top-[2px] h-[18px] w-[18px] rounded-full bg-white shadow transition ${checked ? "left-[18px]" : "left-[2px]"}`} />
    </button>
  );
}
...
function Card({ title, children, className = "" }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <section className={`rounded-[9px] border border-[#e6e9ec] bg-white px-[16px] py-[14px] shadow-[0_1px_3px_rgba(15,23,42,.025)] ${className}`}>
      <h3 className="text-[12px] font-extrabold text-[#17234a]">{title}</h3>
...
function Header({ step }: { step: Step }) {
  return (
    <>
      <header>
        <h1 className="text-[26px] font-extrabold leading-none tracking-[-0.03em] text-[#075b33]">Add New Category</h1>
        <nav className="mt-[9px] flex items-center gap-[8px] text-[10.5px] font-semibold text-[#1d2b58]">
          <span>Dashboard</span><span className="text-[#7b8597]">›</span><span>Categories</span><span className="text-[#7b8597]">›</span><span>Add New Category</span>
          {step === 5 ? <><span className="text-[#7b8597]">›</span><span>Success</span></> : null}
        </nav>
      </header>

      {step !== 5 ? (
        <section className="mt-[16px] rounded-[9px] border border-[#e6e9ec] bg-white px-[18px] py-[13px]">
              ...
              return (
                <div key={item.step} className="flex items-center">
                  <div className="flex min-w-0 items-center gap-[10px]">
                    <div className={`grid h-[28px] w-[28px] shrink-0 place-items-center rounded-full text-[10px] font-extrabold ${done || active ? "bg-[#075b33] text-white" : "bg-[#eef1f6] text-[#34425e]"}`}>
                      {done ? <Check className="h-[14px] w-[14px]" /> : item.step}
                    </div>
                    <div className="min-w-0">
                      <p className={`truncate text-[10px] font-extrabold ${active ? "text-[#075b33]" : "text-[#1d2b58]"}`}>{item.title}</p>
                      <p className="mt-[3px] truncate text-[8.6px] font-semibold text-[#65728b]">{item.subtitle}</p>
                    </div>
                  </div>
...
function CheckLine({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-[8px]">
      <span className="mt-[1px] grid h-[15px] w-[15px] shrink-0 place-items-center rounded-full bg-[#0b6a3b] text-white"><Check className="h-[9px] w-[9px]" /></span>
      <span className="text-[9px] font-semibold leading-[1.45] text-[#53617c]">{children}</span>
    </div>
  );
}
  ...
  return (
    <section className={`rounded-[9px] border px-[18px] py-[16px] ${blue ? "border-[#dce8f6] bg-[#f7faff]" : "border-[#f0e5c8] bg-[#fffaf0]"}`}>
      <div className="flex items-start gap-[12px]">
        ...
        <div>
          <h3 className={`text-[12px] font-extrabold ${blue ? "text-[#1d3d89]" : "text-[#78501c]"}`}>Need Help?</h3>
          <p className="mt-[5px] text-[9px] font-semibold text-[#53617c]">For any assistance, contact our team.</p>
          <div className="mt-[12px] space-y-[9px]">
            <div className="flex items-center gap-[9px]"><Phone className="h-[14px] w-[14px] text-[#147044]" /><span className="text-[10px] font-extrabold text-[#087342]">+91 98765 43210</span></div>
            <div className="flex items-center gap-[9px]"><Mail className="h-[14px] w-[14px] text-[#147044]" /><span className="text-[9.5px] font-extrabold text-[#1f3270]">support@mokshasewa.org</span></div>
          </div>
        </div>
      </div>
    </section>
    ...
    <Card title="Category Preview">
      <div className="flex items-center gap-[14px]">
        ...
        <div>
          ...
          <span className={`mt-[8px] inline-flex rounded-[4px] px-[8px] py-[3px] text-[8px] font-bold ${published ? "bg-emerald-100 text-emerald-700" : "bg-[#eef1f5] text-[#58657c]"}`}>{published ? "Published" : "Draft"}</span>
        </div>
      </div>
    </Card>
  );
    ...
    <main style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }} className="h-full min-h-0 w-full overflow-y-auto overflow-x-hidden bg-[#fffefb] px-[16px] py-[12px] text-[#142347] [&::-webkit-scrollbar]:w-[6px] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300">
      ...
      {step === 1 && (
        <>
            ...
            <section className="overflow-hidden rounded-[9px] border border-[#e6e9ec] bg-white">
              ...
              <div className="p-[18px]">
                <div className="grid grid-cols-2 gap-x-[22px] gap-y-[16px]">
                  <div><FieldLabel required>Category Name</FieldLabel><input value={form.name} onChange={e => update("name", e.target.value.slice(0, 100))} placeholder="Enter category name" className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] px-[12px] text-[10px] font-semibold outline-none placeholder:text-[#8d97aa]" /><p className="mt-[6px] text-right text-[8.5px] font-semibold text-[#718096]">{form.name.length} / 100</p></div>
                  <div><FieldLabel>Parent Category</FieldLabel><select value={form.parentCategory} onChange={e => update("parentCategory", e.target.value)} className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] px-[12px] text-[10px] font-semibold text-[#42506b] outline-none"><option value="">Select Parent Category</option><option>Awareness</option><option>Guidance</option><option>Moksha Sewa</option><option>Stories</option></select><p className="mt-[6px] text-[8.5px] font-semibold text-[#718096]">Choose a parent category (if applicable).</p></div>
                  <div><FieldLabel required>Slug (URL Friendly)</FieldLabel><input value={form.slug} onChange={e => update("slug", e.target.value.slice(0, 100))} placeholder="enter-category-slug" className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] px-[12px] text-[10px] font-semibold outline-none placeholder:text-[#8d97aa]" /><div className="mt-[6px] flex justify-between"><p className="text-[8.5px] font-semibold text-[#718096]">Use lowercase letters, numbers and hyphens only.</p><span className="text-[8.5px] font-semibold text-[#718096]">{form.slug.length} / 100</span></div></div>
                  <div><FieldLabel>Short Description</FieldLabel><textarea value={form.shortDescription} onChange={e => update("shortDescription", e.target.value.slice(0, 160))} placeholder="Enter short description" className="h-[72px] w-full resize-none rounded-[6px] border border-[#dfe4e8] px-[12px] py-[10px] text-[10px] font-semibold outline-none placeholder:text-[#8d97aa]" /><div className="mt-[6px] flex justify-between"><p className="text-[8.5px] font-semibold text-[#718096]">A brief description to display with the category.</p><span className="text-[8.5px] font-semibold text-[#718096]">{form.shortDescription.length} / 160</span></div></div>
                  <div className="col-span-2"><FieldLabel>Category Description</FieldLabel><div className="overflow-hidden rounded-[6px] border border-[#dfe4e8] bg-white"><div className="flex h-[38px] items-center gap-[14px] border-b border-[#e8ebee] px-[12px] text-[#33415d]"><button className="text-[9px] font-bold">Paragraph</button><b>B</b><i>I</i><u>U</u><List className="h-[14px] w-[14px]" /><Link2 className="h-[14px] w-[14px]" /><ImageIcon className="h-[14px] w-[14px]" /><Maximize2 className="h-[14px] w-[14px]" /></div><textarea value={form.description} onChange={e => update("description", e.target.value.slice(0, 2000))} placeholder="Enter detailed description about this category..." className="h-[96px] w-full resize-none px-[12px] py-[10px] text-[10px] font-semibold outline-none placeholder:text-[#8d97aa]" /><div className="flex h-[24px] items-center justify-end px-[10px] text-[8px] font-semibold text-[#718096]">{form.description.length} / 2000</div></div></div>
                  {["Category Icon", "Category Image"].map((label, i) => <div key={label}><FieldLabel optional>{label}</FieldLabel><button className="flex h-[115px] w-full items-center justify-center gap-[16px] rounded-[7px] border border-[#dfe4e8] bg-white">{i === 0 ? <Upload className="h-[34px] w-[34px] text-[#087342]" /> : <ImageIcon className="h-[34px] w-[34px] text-[#087342]" />}<div className="text-left"><p className="text-[9.5px] font-bold text-[#43516a]">Drag & drop a {i === 0 ? "icon" : "image"} here or click to upload</p><p className="mt-[4px] text-[8.5px] font-semibold text-[#718096]">{i === 0 ? "SVG, PNG or JPG" : "JPG, PNG or WEBP"} (Max 2MB)</p><span className="mt-[9px] inline-flex h-[32px] items-center rounded-[5px] border border-[#dfe4e8] px-[12px] text-[8.5px] font-bold text-[#26344f]">Choose File</span></div></button></div>)}
                </div>
              </div>
            </section>
          ...
          <footer className="sticky bottom-0 z-20 mt-[14px] flex min-h-[58px] items-center justify-between border-t border-[#edf0f2] bg-[#fffefb]/95 px-[6px] py-[8px] backdrop-blur-sm"><button className="inline-flex h-[40px] items-center gap-[8px] rounded-[6px] border border-[#dfe3e7] bg-white px-[18px] text-[10px] font-bold text-[#273655]"><X className="h-[14px] w-[14px]" />Cancel</button><button onClick={next} className="inline-flex h-[40px] items-center gap-[8px] rounded-[6px] bg-[#075b33] px-[22px] text-[10px] font-bold text-white">Next<ArrowRight className="h-[14px] w-[14px]" /></button></footer>
        </>
      )}
      ...
      {step === 3 && (
        <>
          <section className="mt-[14px] grid items-start gap-[14px] xl:grid-cols-[minmax(0,1fr)_315px]">
            <section className="overflow-hidden rounded-[9px] border border-[#e6e9ec] bg-white">
              <div className="flex min-h-[72px] items-center gap-[12px] bg-[linear-gradient(90deg,#eef7f1,#f8fbf8)] px-[18px]"><div className="grid h-[44px] w-[44px] place-items-center rounded-full bg-[#dff1e5] text-[#087342]"><Search className="h-[24px] w-[24px]" /></div><div><h2 className="text-[14px] font-extrabold text-[#175b37]">3. SEO Settings</h2><p className="mt-[4px] text-[9.5px] font-semibold text-[#52617a]">Optimize this category for better visibility on search engines.</p></div></div>
              <div className="grid grid-cols-2 divide-x divide-[#edf0f2] p-[18px]">
                <div className="pr-[22px] space-y-[18px]">
                  <div><FieldLabel required>SEO Meta Title</FieldLabel><input value={form.seoTitle} onChange={e => update("seoTitle", e.target.value.slice(0, 60))} placeholder="Enter SEO title for this category" className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] px-[12px] text-[9.5px] font-semibold outline-none placeholder:text-[#8d97aa]" /><div className="mt-[6px] flex justify-between"><span className="text-[8.5px] font-semibold text-[#718096]">Recommended length: 50–60 characters</span><span className="text-[8.5px] font-semibold text-[#718096]">{form.seoTitle.length} / 60</span></div></div>
                  <div><FieldLabel required>SEO Meta Description</FieldLabel><textarea value={form.seoDescription} onChange={e => update("seoDescription", e.target.value.slice(0, 160))} placeholder="Enter SEO meta description for this category" className="h-[112px] w-full resize-none rounded-[6px] border border-[#dfe4e8] px-[12px] py-[10px] text-[9.5px] font-semibold outline-none placeholder:text-[#8d97aa]" /><div className="mt-[6px] flex justify-between"><span className="text-[8.5px] font-semibold text-[#718096]">Recommended length: 150–160 characters</span><span className="text-[8.5px] font-semibold text-[#718096]">{form.seoDescription.length} / 160</span></div></div>
                  <div><FieldLabel optional>SEO Keywords</FieldLabel><input value={form.seoKeywords} onChange={e => update("seoKeywords", e.target.value)} placeholder="Enter keywords separated by commas" className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] px-[12px] text-[9.5px] font-semibold outline-none placeholder:text-[#8d97aa]" /><p className="mt-[6px] text-[8.5px] font-semibold text-[#718096]">Example: moksha sewa, final journey, rituals, volunteer</p></div>
                  <div><FieldLabel optional>Canonical URL</FieldLabel><input value={form.canonicalUrl} onChange={e => update("canonicalUrl", e.target.value)} className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] bg-[#f1f4f5] px-[12px] text-[9.5px] font-semibold text-[#66738b] outline-none" /><p className="mt-[6px] text-[8.5px] font-semibold text-[#718096]">Leave blank to use the default URL.</p></div>
                  <div><FieldLabel>Schema Markup</FieldLabel><button onClick={() => update("schemaEnabled", !form.schemaEnabled)} className="flex items-start gap-[10px] text-left"><span className={`mt-[1px] grid h-[16px] w-[16px] place-items-center rounded-[3px] border ${form.schemaEnabled ? "border-[#087342] bg-[#087342] text-white" : "border-[#cbd3dd]"}`}><Check className="h-[10px] w-[10px]" /></span><div><p className="text-[9.5px] font-extrabold text-[#24345e]">Enable structured data (Category Schema)</p><p className="mt-[3px] text-[8.5px] font-semibold text-[#6d7890]">Helps search engines understand your content better.</p></div></button></div>
                </div>
                <div className="pl-[22px]">
                  <h3 className="text-[12px] font-extrabold text-[#17234a]">Social Media Preview (Open Graph)</h3><p className="mt-[4px] text-[8.8px] font-semibold text-[#66738b]">This image and content will be used when the category is shared on social media.</p>
                  <div className="mt-[16px]"><FieldLabel optional>Social Image</FieldLabel><button className="flex h-[112px] w-full items-center justify-center gap-[16px] rounded-[7px] border border-[#dfe4e8]"><ImageIcon className="h-[34px] w-[34px] text-[#087342]" /><div className="text-left"><p className="text-[9.5px] font-bold text-[#43516a]">Drag & drop an image here or click to upload</p><p className="mt-[4px] text-[8.5px] font-semibold text-[#718096]">JPG, PNG or WEBP (Recommended size: 1200x630px)</p><span className="mt-[9px] inline-flex h-[32px] items-center rounded-[5px] border border-[#dfe4e8] px-[12px] text-[8.5px] font-bold text-[#26344f]">Choose File</span></div></button></div>
                  <div className="mt-[16px]"><FieldLabel optional>Social Title</FieldLabel><input value={form.socialTitle} onChange={e => update("socialTitle", e.target.value.slice(0, 60))} placeholder="Enter social media title" className="h-[40px] w-full rounded-[6px] border border-[#dfe4e8] px-[12px] text-[9.5px] font-semibold outline-none placeholder:text-[#8d97aa]" /><p className="mt-[6px] text-right text-[8.5px] font-semibold text-[#718096]">{form.socialTitle.length} / 60</p></div>
                  <div className="mt-[14px]"><FieldLabel optional>Social Description</FieldLabel><textarea value={form.socialDescription} onChange={e => update("socialDescription", e.target.value.slice(0, 160))} placeholder="Enter social media description" className="h-[76px] w-full resize-none rounded-[6px] border border-[#dfe4e8] px-[12px] py-[10px] text-[9.5px] font-semibold outline-none placeholder:text-[#8d97aa]" /><p className="mt-[6px] text-right text-[8.5px] font-semibold text-[#718096]">{form.socialDescription.length} / 160</p></div>
                  <div className="mt-[18px] rounded-[8px] border border-[#e3e7eb] p-[14px]"><div className="flex items-center gap-[9px]"><Share2 className="h-[17px] w-[17px] text-[#2e65d0]" /><h3 className="text-[11px] font-extrabold text-[#17234a]">Google Search Preview</h3></div><p className="mt-[12px] text-[8.5px] font-semibold text-[#66738b]">https://www.mokshasewa.org › category › {finalSlug}</p><p className="mt-[7px] text-[15px] font-extrabold text-[#2343a3]">{form.seoTitle || "Category Title Will Appear Here"}</p><p className="mt-[5px] text-[8.8px] font-semibold leading-[1.5] text-[#526078]">{form.seoDescription || "This is how your category may appear on Google search results. Write a clear and engaging meta description to get better visibility."}</p></div>
                </div>
              </div>
            </section>
            <aside className="space-y-[12px]"><section className="rounded-[9px] border border-[#dfe9e2] bg-[#f8fcf9] px-[18px] py-[16px]"><div className="flex items-center gap-[10px]"><Lightbulb className="h-[21px] w-[21px] text-[#d5a31d]" /><h3 className="text-[12px] font-extrabold text-[#1b633e]">SEO Best Practices</h3></div><div className="mt-[14px] space-y-[10px]"><CheckLine>Use a clear and relevant meta title.</CheckLine><CheckLine>Write a unique and engaging description.</CheckLine><CheckLine>Include important keywords.</CheckLine><CheckLine>Add an attractive social media image.</CheckLine><CheckLine>Keep it relevant to the category content.</CheckLine><CheckLine>Preview how it looks in search results.</CheckLine></div></section><Card title="Category Progress"><div className="space-y-[12px]">{steps.map(s => <div key={s.step} className="flex items-center gap-[10px]"><span className={`grid h-[23px] w-[23px] place-items-center rounded-full text-[9px] font-extrabold ${step > s.step ? "bg-[#075b33] text-white" : step === s.step ? "bg-[#18a84f] text-white" : "bg-[#e8ebf1] text-[#68758d]"}`}>{step > s.step ? <Check className="h-[12px] w-[12px]" /> : s.step}</span><div><p className="text-[9.5px] font-extrabold text-[#24345e]">{s.title}</p><span className={`mt-[3px] inline-flex rounded-[4px] px-[7px] py-[3px] text-[7.8px] font-bold ${step > s.step ? "bg-emerald-100 text-emerald-700" : step === s.step ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}`}>{step > s.step ? "Completed" : step === s.step ? "In Progress" : "Pending"}</span></div></div>)}</div></Card><HelpCard blue /></aside>
          </section>
          <footer className="sticky bottom-0 z-20 mt-[14px] flex min-h-[58px] items-center justify-between border-t border-[#edf0f2] bg-[#fffefb]/95 px-[6px] py-[8px] backdrop-blur-sm"><button onClick={back} className="inline-flex h-[40px] items-center gap-[8px] rounded-[6px] border border-[#dfe3e7] bg-white px-[18px] text-[10px] font-bold text-[#273655]"><ArrowLeft className="h-[14px] w-[14px]" />Back</button><div className="flex gap-[10px]"><button className="inline-flex h-[40px] items-center gap-[8px] rounded-[6px] border border-[#dfe3e7] bg-white px-[18px] text-[10px] font-bold text-[#273655]"><FileText className="h-[14px] w-[14px]" />Save as Draft</button><button onClick={next} className="inline-flex h-[40px] items-center gap-[8px] rounded-[6px] bg-[#075b33] px-[22px] text-[10px] font-bold text-white">Next<ArrowRight className="h-[14px] w-[14px]" /></button></div></footer>
        </>
      )}
          ...
          <section className="mt-[14px] grid items-start gap-[14px] xl:grid-cols-[minmax(0,1fr)_315px]">
            <section className="overflow-hidden rounded-[9px] border border-[#e6e9ec] bg-white">
              ...
              <div className="grid grid-cols-2 gap-[14px] p-[18px]">
                <Card title="Category Details"><div className="space-y-[8px]">{[["Category Name", finalName], ["Slug (URL Friendly)", finalSlug], ["Parent Category", finalParent], ["Short Description", finalShort]].map(([a, b]) => <div key={a} className="grid grid-cols-[135px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">{a}</span><span className="text-[9px] font-bold text-[#34425e]">{b}</span></div>)}<div className="grid grid-cols-[135px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">Description</span><span className="text-[8.8px] font-semibold leading-[1.5] text-[#34425e]">{finalDescription}</span></div><div className="grid grid-cols-[135px_1fr] items-center gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">Category Icon</span><span className="flex items-center gap-[8px] text-[9px] font-bold text-[#34425e]"><Heart className="h-[18px] w-[18px] text-[#087342]" />heart (icon)</span></div></div></Card>
                <Card title="Display & Settings"><div className="space-y-[8px]">{[["Display in Navigation Menu", form.displayInNavigation ? "Yes" : "No"], ["Menu Label", form.menuLabel || "Stories"], ["Parent Menu Position", form.parentMenuPosition || "Blog & Awareness"], ["Display Order", form.order], ["Featured Category", form.featured ? "Yes" : "No"]].map(([a, b]) => <div key={a} className="grid grid-cols-[145px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">{a}</span><span className="text-[9px] font-bold text-[#34425e]">{b}</span></div>)}<div className="grid grid-cols-[145px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">Category Visibility</span><span><span className="rounded-[4px] bg-emerald-100 px-[8px] py-[3px] text-[8px] font-bold text-emerald-700">{form.visibility}</span></span></div><div className="grid grid-cols-[145px_1fr] items-center gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">Category Color</span><span className="flex items-center gap-[8px] text-[9px] font-bold text-[#34425e]"><span className="h-[16px] w-[16px] rounded-full" style={{ backgroundColor: form.color }} />{form.color}</span></div><div className="grid grid-cols-[145px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">Category Banner</span><div className="relative h-[78px] overflow-hidden rounded-[6px] bg-[linear-gradient(180deg,#f6eac5,#cbd7b5)]"><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_90%,rgba(255,215,117,.75),transparent_30%),linear-gradient(to_top,rgba(38,93,59,.45),transparent_55%)]" /><div className="absolute inset-x-0 top-[26px] text-center text-[17px] font-semibold italic text-[#315f41]">Stories That Inspire</div></div></div></div></Card>
                <Card title="SEO Settings"><div className="space-y-[8px]">{[["SEO Meta Title", finalSeoTitle], ["SEO Meta Description", finalSeoDescription], ["SEO Keywords", finalKeywords], ["Canonical URL", `${form.canonicalUrl}${finalSlug}/`]].map(([a, b]) => <div key={a} className="grid grid-cols-[135px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">{a}</span><span className="text-[8.8px] font-semibold leading-[1.45] text-[#34425e]">{b}</span></div>)}<div className="grid grid-cols-[135px_1fr] gap-[12px]"><span className="text-[8.8px] font-semibold text-[#66738b]">Schema Markup</span><span className="flex items-center gap-[7px] text-[9px] font-bold text-[#34425e]"><span className="grid h-[16px] w-[16px] place-items-center rounded-[3px] bg-[#087342] text-white"><Check className="h-[10px] w-[10px]" /></span>Enabled</span></div></div></Card>
                <Card title="Social Media Preview"><div className="relative h-[104px] overflow-hidden rounded-[7px] bg-[linear-gradient(180deg,#f6eac5,#cbd7b5)]"><div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_90%,rgba(255,215,117,.75),transparent_30%),linear-gradient(to_top,rgba(38,93,59,.45),transparent_55%)]" /><div className="absolute inset-x-0 top-[36px] text-center text-[21px] font-semibold italic text-[#315f41]">Stories That Inspire</div></div><p className="mt-[8px] text-[8.5px] font-semibold text-[#66738b]">mokshasewa.org</p><p className="mt-[4px] text-[10px] font-extrabold text-[#1f2f57]">{finalSeoTitle}</p><p className="mt-[4px] line-clamp-2 text-[8.5px] font-semibold leading-[1.45] text-[#66738b]">{finalSeoDescription}</p></Card>
              </div>
            </section>
            <aside className="space-y-[12px]"><section className="rounded-[9px] border border-[#dfe9e2] bg-[#f8fcf9] px-[18px] py-[16px]"><div className="flex items-center gap-[10px]"><CheckCircle2 className="h-[21px] w-[21px] text-[#087342]" /><h3 className="text-[12px] font-extrabold text-[#1b633e]">Checklist</h3></div><div className="mt-[14px] space-y-[10px]"><CheckLine>Category details added</CheckLine><CheckLine>Display settings configured</CheckLine><CheckLine>SEO settings completed</CheckLine><CheckLine>All information reviewed</CheckLine><CheckLine>Ready to publish</CheckLine></div></section><Card title="What Happens Next?"><div className="space-y-[11px]">{["The category will be saved and published.", "It will appear in the navigation menu (if enabled).", "You can now add posts under this category.", "You can edit or update the category anytime."].map((t, i) => <div key={t} className="flex items-start gap-[10px]"><span className="grid h-[24px] w-[24px] shrink-0 place-items-center rounded-full bg-[#dff1e5] text-[9px] font-extrabold text-[#25633f]">{i + 1}</span><p className="pt-[4px] text-[8.8px] font-semibold leading-[1.4] text-[#53617c]">{t}</p></div>)}</div></Card><HelpCard blue /></aside>
          </section>
      ...
      {step === 5 && (
        <section className="mt-[16px] grid items-start gap-[14px] xl:grid-cols-[minmax(0,1fr)_315px]">
          <div className="space-y-[14px]">
            <section className="overflow-hidden rounded-[9px] border border-[#dfe9e2] bg-[linear-gradient(135deg,#f4fbf6,#fbfdfb)] px-[22px] py-[30px] text-center"><div className="mx-auto grid h-[110px] w-[110px] place-items-center rounded-full bg-[linear-gradient(135deg,#26b943,#3ca837)] text-white"><Check className="h-[58px] w-[58px]" strokeWidth={3.4} /></div><h2 className="mt-[20px] text-[29px] font-extrabold tracking-[-0.03em] text-[#075b33]">Category Added Successfully!</h2><p className="mt-[9px] text-[12px] font-semibold text-[#2e3d68]">“{finalName}” has been added to your blog & awareness categories.</p><p className="mt-[6px] text-[10px] font-semibold text-[#5f6c82]">You can now create and publish posts under this category.</p><div className="mt-[28px] flex flex-wrap justify-center gap-[12px]"><button onClick={() => setStep(1)} className="inline-flex h-[46px] items-center gap-[9px] rounded-[7px] bg-[#075b33] px-[22px] text-[10px] font-bold text-white"><Plus className="h-[15px] w-[15px]" />Add Another Category</button><button className="inline-flex h-[46px] items-center gap-[9px] rounded-[7px] border border-[#a9c8b6] bg-white px-[22px] text-[10px] font-bold text-[#1d5d3c]"><FileText className="h-[15px] w-[15px]" />Create a New Post</button><button className="inline-flex h-[46px] items-center gap-[9px] rounded-[7px] border border-[#a9c8b6] bg-white px-[22px] text-[10px] font-bold text-[#1d5d3c]"><List className="h-[15px] w-[15px]" />View All Categories</button><button className="inline-flex h-[46px] items-center gap-[9px] rounded-[7px] border border-[#a9c8b6] bg-white px-[22px] text-[10px] font-bold text-[#1d5d3c]"><ExternalLink className="h-[15px] w-[15px]" />Visit Website</button></div></section>
            <Card title="Category Summary"><div className="grid grid-cols-4 gap-[10px]">{[["Total Categories", "13", "+1 from last entry", BookOpen, "bg-emerald-50 text-emerald-700"], ["Published Categories", "11", "", FileText, "bg-violet-50 text-violet-700"], ["Hidden Categories", "2", "", Eye, "bg-amber-50 text-amber-700"], ["Total Posts", "92", "Across all categories", FileText, "bg-blue-50 text-blue-700"]].map(([label, value, note, Icon, tone]) => { const I = Icon as typeof BookOpen; return <div key={String(label)} className="flex min-h-[92px] items-center gap-[12px] rounded-[8px] border border-[#edf0f2] px-[12px]"><div className={`grid h-[46px] w-[46px] place-items-center rounded-full ${String(tone)}`}><I className="h-[22px] w-[22px]" /></div><div><p className="text-[8.5px] font-bold text-[#66738b]">{String(label)}</p><p className="mt-[4px] text-[20px] font-extrabold text-[#142347]">{String(value)}</p>{note ? <p className="mt-[4px] text-[8px] font-semibold text-[#66738b]">{String(note)}</p> : null}</div></div> })}</div></Card>
            <Card title="Recently Added Categories"><div className="overflow-x-auto"><table className="w-full min-w-[760px] border-collapse text-left"><thead><tr className="h-[36px] border-b border-[#edf0f2] text-[8px] font-extrabold uppercase tracking-[0.04em] text-[#536078]"><th className="px-[8px]">#</th><th className="px-[8px]">Category Name</th><th className="px-[8px]">Slug</th><th className="px-[8px]">Parent Category</th><th className="px-[8px]">Status</th><th className="px-[8px]">Date</th><th className="px-[8px] text-center">Actions</th></tr></thead><tbody>{recent.map((r, i) => <tr key={r[0]} className="h-[44px] border-b border-[#f0f2f4] last:border-b-0"><td className="px-[8px] text-[8.8px] font-bold text-[#35445f]">{i + 1}</td><td className="px-[8px] text-[9px] font-extrabold text-[#1e2d52]">{r[0]}</td><td className="px-[8px] text-[8.6px] font-semibold text-[#53617c]">{r[1]}</td><td className="px-[8px] text-[8.6px] font-semibold text-[#365b9e]">{r[2]}</td><td className="px-[8px]"><span className={`rounded-[4px] px-[8px] py-[3px] text-[7.8px] font-bold ${r[3] === "Published" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{r[3]}</span></td><td className="px-[8px] text-[8.6px] font-semibold text-[#53617c]">{r[4]}</td><td className="px-[8px]"><div className="flex justify-center gap-[12px]"><Eye className="h-[13px] w-[13px] text-[#2e3f73]" /><Edit3 className="h-[13px] w-[13px] text-[#2e3f73]" /><Trash2 className="h-[13px] w-[13px] text-[#d64242]" /></div></td></tr>)}</tbody></table></div></Card>
          </div>
