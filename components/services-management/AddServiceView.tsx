...
const fieldClass = "h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-[11.5px] font-medium text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100";
...
function Section({ number, title, note, children, className = "" }: { number: number; title: string; note: string; children: React.ReactNode; className?: string }) {
  return <section className={`rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_3px_rgba(15,23,42,.025)] ${className}`}><div className="mb-4"><h2 className="text-[14px] font-bold text-slate-950">{number}. {title}</h2><p className="mt-1 text-[11.5px] text-slate-500">{note}</p></div>{children}</section>;
}

function Field({ label, required, hint, children }: { label: string; required?: boolean; hint?: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-1.5 block text-[10.5px] font-bold text-slate-800">{label}{required && <b className="ml-1 text-red-500">*</b>}</span>{children}{hint && <small className="mt-1.5 block text-[9.5px] text-slate-500">{hint}</small>}</label>;
}
        ...
        <div className="mt-2 grid items-start gap-1.5 xl:grid-cols-[minmax(0,1.55fr)_minmax(350px,1fr)]">
          <Section number={1} title="Service Information" note="Provide the basic details of the service.">
            <div className="space-y-3.5">
              ...
              <Field label="Detailed Description" required><div className="overflow-hidden rounded-lg border border-slate-200"><div className="flex min-h-9 flex-wrap items-center gap-0.5 border-b border-slate-200 bg-slate-50 px-2"><select className="mr-2 h-7 border-0 bg-transparent text-[10px] font-medium outline-none"><option>Paragraph</option></select>{[Bold, Italic, Underline, Strikethrough, Quote, List, ListOrdered, AlignLeft, AlignCenter, AlignRight, Link2, ImageIcon, Undo2, Redo2].map((Icon, index) => <button key={index} type="button" className="grid size-7 place-items-center rounded hover:bg-white"><Icon size={13} /></button>)}</div><textarea rows={5} placeholder="Write detailed description of the service..." className="w-full resize-none border-0 px-3 py-3 text-[11.5px] outline-none" /><div className="border-t border-slate-100 px-3 py-1.5 text-[9px] text-slate-500">Word count: 0</div></div></Field>
